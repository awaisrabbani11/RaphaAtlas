...
:root{
  ...
  --radius:16px;
  --radius-sm:11px;
  --shadow:0 1px 2px rgba(20,48,43,.05),0 12px 34px -18px rgba(20,48,43,.28);
...
<div class="ra-conception">
  <div class="grid">
    <section class="card pad" aria-label="Calculator inputs">
      ...
      <div class="methods" role="group" aria-label="Calculation method">
        <button class="method" data-method="edd" aria-pressed="true">Due date</button>
        <button class="method" data-method="lmp" aria-pressed="false">Last period</button>
        <button class="method" data-method="us" aria-pressed="false">Ultrasound</button>
        <button class="method" data-method="ivf" aria-pressed="false">IVF transfer</button>
      ...
      <div class="field" id="asofWrap">
        ...
        <p class="hint">Defaults to today. Change it to date a past or future point.</p>
      </div>
    ...
    <div class="grid">
      <div>
        ...
        <div class="note">
          ...
          <p>Gestational age is counted from the first day of your last period. Fetal (conceptional) age is counted from conception — about two weeks less. Most medical timelines use gestational age.</p>
        </div>
      </div>
      <div>
        <div class="note">
...
<p class="mb-4 font-body-md text-on-surface">A developing baby cannot announce the day it began, so competent practice works backward. We read your last period date, an ultrasound date, or an IVF transfer date toward the true date of conception instead.</p>
<p class="mb-4 font-body-md text-on-surface">Clinicians rarely trust one method alone. Your LMP, your ultrasound results, and your fertility timing each estimate the approximate conception date differently, and honest counseling means admitting these figures vary from person to person noticeably.</p>
<p class="mb-4 font-body-md text-on-surface">Why does knowing how far along you are matter so much? Because a defensible gestational age shapes every prenatal decision ahead, from screening windows to the expected due date your provider will eventually calculate together.</p>
<p class="mb-4 font-body-md text-on-surface">Treat the Conception Calculator as a compass, never a guarantee of timing. It estimates when you likely conceived, offering a reliable estimate and corresponding due date rather than a promise about your actual birth date.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">When Did I Conceive / When Did I Get Pregnant</h2>
<p class="mb-4 font-body-md text-on-surface">Forget the tidy assumption that conception matches the night of sexual intercourse. Sperm stays viable several days inside you, so fertilization may occur far later, quietly widening the plausible range of possible conception dates here.</p>
<p class="mb-4 font-body-md text-on-surface">Reverse arithmetic anchors most estimation here. Since a 40-week pregnancy counts roughly 266 days from conception, we subtract backward from any estimated due date, producing an estimated conception date your healthcare provider can independently sanity-check.</p>
<p class="mb-4 font-body-md text-on-surface">Your cycle length quietly rewrites everything. Textbook logic assumes a regular 28-day cycle with ovulation on day 14, yet irregular periods, longer phases, and shorter ones each shift when conception genuinely occurred within your body.</p>
<p class="mb-4 font-body-md text-on-surface">Numbers make this concrete. A 25-day cycle places ovulation near day 11, while a 35-day cycle pushes it toward day 21, so a corrected LMP matters before trusting any estimated conception date blindly, frankly, today.</p>
<p class="mb-4 font-body-md text-on-surface">Practitioners weigh a sonogram against dates. When your first day memory is fuzzy, an early ultrasound measuring gestational age overrides shaky counting, because accurate recall of a regular period frankly fails many patients seeking honesty.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Last Menstrual Period (LMP)</h2>
<p class="mb-4 font-body-md text-on-surface">The Last Menstrual Period dominates because it is simple, not because it is truly accurate. Providers still adore it, yet LMP silently assumes textbook normal cycles that most real living human bodies never actually keep.</p>
<p class="mb-4 font-body-md text-on-surface">Mechanically, the LMP method takes the first day of your last period and applies add 280 days, roughly 40 weeks or 9 months and 7 days, generating a fast, defensible due date almost instantly today.</p>
<p class="mb-4 font-body-md text-on-surface">Here lies the buried flaw everyone ignores. The formula presumes ovulation at day 14 of a regular cycle, so anyone with irregular cycles or unusual cycle lengths inherits a quietly wrong gestational age immediately onward.</p>
<p class="mb-4 font-body-md text-on-surface">Correction beats blind faith. Using Conception Date = LMP + (Cycle Length - 14 days) lets us subtract 14 days intelligently, adjusting the number of days whenever your average cycle length strays past 28 days.</p>
<p class="mb-4 font-body-md text-on-surface">Boundaries matter clinically here. Normal cycles span 21 to 35 days, and beyond that range the LMP estimate frays badly. That is precisely why thoughtful family planning pairs recall with ultrasound, never recall alone honestly.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Due Date Method</h2>
<p class="mb-4 font-body-md text-on-surface">Working from a known due date feels backward, yet it is often the cleanest path. If a sonogram already fixed your EDD, we simply reverse the math toward a corresponding window of possible conception dates.</p>
<p class="mb-4 font-body-md text-on-surface">The arithmetic stays blunt. Conception Date = Due Date - 266 Days reflects that a full 40-week pregnancy runs 2 weeks shorter in real embryonic terms, landing near 38 weeks of genuine post-fertilization growth inside.</p>
<p class="mb-4 font-body-md text-on-surface">Worked example beats abstraction every time. Take a due date of August 9; subtract the 266 days and you arrive around November 16 of the previous year as the most plausible estimated conception date overall.</p>
<p class="mb-4 font-body-md text-on-surface">This method shines when earlier data exists already. A confirmed IVF transfer or a first-trimester ultrasound yields a sturdier due date than shaky LMP recall, so back-calculating from it produces a more accurate final answer.</p>
<p class="mb-4 font-body-md text-on-surface">Never oversell the output though, ever. Even a polished Due Date Calculation delivers a range, not a verdict, because natural variation means barely 5% of babies actually arrive on the precise estimated due date printed.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Conception Date Method</h2>
<p class="mb-4 font-body-md text-on-surface">Forget the notion that one remembered night pins your conception date. Because live sperm linger inside the reproductive tract, real fertilization can arrive five days earlier than the intercourse you keep blaming and counting on.</p>
<p class="mb-4 font-body-md text-on-surface">The cleanest Conception Date Method runs backward from a known due date, since the sound calculation adds 266 days from fertilization rather than counting from your imprecise last menstrual period and its usual two-week padding.</p>
<p class="mb-4 font-body-md text-on-surface">I trust this most when a patient logged ovulation deliberately. Ovulation predictor kits, ovulation test strips, and a disciplined basal body temperature chart mark the ovulation window far tighter than any generic assumed 28-day cycle.</p>
<p class="mb-4 font-body-md text-on-surface">Fertility treatments rewrite the arithmetic completely. Under IUI or in vitro fertilization, the human egg meets sperm at a scheduled hour, so the brief 12 to 24 hours of viability, not vague memory, fixes conception.</p>
<p class="mb-4 font-body-md text-on-surface">Skeptics demand one exact day, yet biology declines politely. An honest conception date is really a range of days, anchored wherever ovulation occurs, then quietly confirmed later through ultrasound dating instead of the wall calendar.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Ultrasound / Gestational Age</h2>
<p class="mb-4 font-body-md text-on-surface">Your LMP date is a guess wearing the costume of fact, and the obstetric ultrasound overrules it without apology. Sound waves assemble images of internal body structures, and those measurements date the pregnancy more honestly.</p>
<p class="mb-4 font-body-md text-on-surface">Early pregnancy ultrasounds earn trust between 6 and 8 weeks, when crown-rump length, read from top of head to bottom, predicts gestational age within 5 to 7 days, the most accurate window you will get.</p>
<p class="mb-4 font-body-md text-on-surface">Practitioners convert a raw crown rump length into weeks and days through a sonographic method, never intuition. A first-trimester scan reading 7 weeks 5 days lets me count backward toward the truly genuine ovulation window.</p>
<p class="mb-4 font-body-md text-on-surface">I have watched one early pregnancy scan quietly adjust a due date by ten days. Once fetal size outpaces the calculated LMP, the ultrasound-based EDD wins, because living tissue grows on a reliable biological schedule.</p>
<p class="mb-4 font-body-md text-on-surface">Past 12 weeks the growing baby varies too widely for precise dating, which is why first trimester timing matters. A pregnancy confirmation ultrasound at five or six weeks merely proves viability, never the exact age.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">IVF / ART Embryo Transfer</h2>
<p class="mb-4 font-body-md text-on-surface">Couples using IVF often assume their dating stays fuzzy, yet the reverse holds. Because the embryo transfer date and embryo age sit documented in a chart, IVF/ART yields obstetrics' single most precise conception timeline overall.</p>
<p class="mb-4 font-body-md text-on-surface">The math hinges entirely on which transfer date occurred. A day-three embryo transfer at the cleavage stage, meaning Day 3, and a day-five embryo transfer at the blastocyst stage, meaning Day 5, carry distinct offsets.</p>
<p class="mb-4 font-body-md text-on-surface">For the estimated due date, clinicians reckon from transfer, never from a phantom period. In Vitro Fertilization records replace the menstrual cycles and ovulation dates muddying natural conception, so the calculator needs one clean number.</p>
<p class="mb-4 font-body-md text-on-surface">A blastocyst embryo transfer adds fewer days than a cleavage transfer, while a Day 6 transfer shifts the sum yet again. This is precisely why I forbid patients reusing a generic pregnancy due date widget.</p>
<p class="mb-4 font-body-md text-on-surface">Fertility treatments strip out the customary adjustment guesswork entirely. Where a natural cycle forces some hazy estimated date range, a logged embryo transfer hands you a fixed origin, rendering gestational age almost stubbornly, accurately knowable.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Reverse-Calculate from Date of Birth</h2>
<p class="mb-4 font-body-md text-on-surface">Working forward toward a due date feels intuitive, yet reversing from an actual date of birth is where precision hides. The formula Conception Date = Date of Birth - 266 Days anchors this entire reconstruction.</p>
<p class="mb-4 font-body-md text-on-surface">The logic rests on gestation in humans spanning roughly 38 to 42 weeks, or 38 x 7 counted days from conception. Full-term pregnancies cluster there, though plus or minus two weeks stays ordinary, never alarming.</p>
<p class="mb-4 font-body-md text-on-surface">Take one date of birth of April 2 1990; the subtraction lands conception near the prior July 10 1989. Repeating that same operation against any birth date exposes the likely conception date with surprising cleanliness.</p>
<p class="mb-4 font-body-md text-on-surface">Because delivery arrives early, late, or precisely on schedule, the reversed figure is a window, not a pinpoint. A June 26 birth against a July 24 birth swings the exact date of conception measurably apart.</p>
<p class="mb-4 font-body-md text-on-surface">Remember the first two weeks of the menstrual cycle predate the true fertilization of the egg entirely. Counting from birth ignores that clinical fiction, delivering an estimated date range grounded in the egg meeting sperm.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Ovulation Method</h2>
<p class="mb-4 font-body-md text-on-surface">Most people assume conception happens the exact moment you have sex, yet fertilization actually follows ovulation by hours, not days. Pinpointing your day of ovulation matters far more than any single bedroom date you half-remember.</p>
<p class="mb-4 font-body-md text-on-surface">Your ovaries release a single egg roughly 14 days before your next period arrives, though real cycles scatter this across an 11-21 days range. That is why lazy cycle length - 28 assumptions wreck estimates.</p>
<p class="mb-4 font-body-md text-on-surface">Once the ovaries let go, the egg travels down the fallopian tubes awaiting sperm. When a sperm fertilizes it, that newly fertilized cell implants into the uterus wall roughly a week afterward, certainly not immediately.</p>
<p class="mb-4 font-body-md text-on-surface">The most commonly used shortcut simply adds 280 days onto your last period, but the ovulation route runs sharper. Count backward, add two weeks onto your calculated ovulation moment, and the fertile window tightens considerably.</p>
<p class="mb-4 font-body-md text-on-surface">I tell patients that tracking ovulation cycles beats memory every single time. Note your menstrual period start, subtract two weeks, and you land on the ovulation day when you were most likely to conceive successfully.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Intercourse / Last Sexual Contact</h2>
<p class="mb-4 font-body-md text-on-surface">Dating a pregnancy from the last time you had sex feels perfectly logical, yet it misleads more couples than it helps. Sperm stay alive far longer than the encounter itself, quietly distorting the whole process.</p>
<p class="mb-4 font-body-md text-on-surface">A man's sperm can survive up to a week inside the reproductive tract, just patiently waiting around. So the day you had intercourse rarely marks the day you genuinely ovulated or actually became pregnant afterward.</p>
<p class="mb-4 font-body-md text-on-surface">This takes time to accept, particularly with more than one partner across a cycle. Because sperm linger, identifying which partner or encounter truly led to conception becomes genuinely difficult without any hard ovulation timing data.</p>
<p class="mb-4 font-body-md text-on-surface">Assuming birth control kept you fully safe is another common trap. No method sits 100% effective at preventing pregnancy, and people who were ovulating while lapsing on protection conceive far more often than they expected.</p>
<p class="mb-4 font-body-md text-on-surface">Your ovaries release the egg into the fallopian tubes, and then can pregnant status begin. Add two weeks past your menstrual period; irregular body cycles and ovulation cycles shift this by two or three days.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">When To Prefer Ultrasound Dating Over LMP</h2>
<p class="mb-4 font-body-md text-on-surface">Trusting your last period to date a pregnancy sounds sensible until you learn how often it misleads prenatal care. LMP dating quietly assumes textbook regular 28-day cycles, yet most real bodies refuse to cooperate neatly.</p>
<p class="mb-4 font-body-md text-on-surface">An early ultrasound scan never guesses; it takes a real-time measure of the fetus to confirm gestational age within a few days. Reading CRL size, the standard procedure across obstetric ultrasounds, reliably beats recalled dates.</p>
<p class="mb-4 font-body-md text-on-surface">The clinical question is discrepancy size, not preference. When your scan and LMP dating diverge past the recommended thresholds, providers redate you. Blindly adding 280 days onto an uncertain period only compounds that original error.</p>
<p class="mb-4 font-body-md text-on-surface">Here are the ACOG bands I actually follow: before 9 0/7 weeks, redate beyond 5 days; across 9 0/7-15 6/7 weeks, seven days; then 16 0/7-21 6/7 weeks loosens to 10 days before correction applies.</p>
<p class="mb-4 font-body-md text-on-surface">Past that, 22 0/7-27 6/7 weeks tolerates 14 days, while 28 0/7 weeks onward permits 21 days. CRL fades beyond 13 weeks 6 days, so the exam date anchors how software back-calculates and derives EDD.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Determining The Estimated Due Date (EDD)</h2>
<p class="mb-4 font-body-md text-on-surface">Your estimated due date offers only a general idea of when you will give birth, never a hard deadline. Treating it as exact timing invites disappointment, since few babies arrive born on exact due date.</p>
<p class="mb-4 font-body-md text-on-surface">The standard clinical method begins by adding 280 days to your last period, setting a rough baseline. That crude midpoint ignores your actual biology, so clinicians refine it well before trusting the final printed number.</p>
...
<p class="mb-4 font-body-md text-on-surface">Mucus tells the forward story. Dry days, then sticky paste, then cloudy mucus, finally clear, slippery, raw egg white strands mark your fertility window. These become the best days for regular lovemaking, sex, or insemination.</p>
<p class="mb-4 font-body-md text-on-surface">Chart both patterns across one full length of the cycle, roughly day 12 through day 17, and a biological pattern emerges. Predicting your next period sharpens chances of conception and best days to get pregnant.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Follow Your Feelings</h2>
<p class="mb-4 font-body-md text-on-surface">Charts can betray you. When the tracking process becomes stressful, it sabotages the very bodies you monitor. Worry and stress flood your system, and the woman's body responds by delaying the favorable hormonal moment entirely.</p>
<p class="mb-4 font-body-md text-on-surface">I tell couples to loosen the charting grip. Have sex roughly every two days as a comfortable minimum, never a chore. This keeps you closer without turning intimacy into a clinical process ruled by spreadsheets.</p>
<p class="mb-4 font-body-md text-on-surface">Feeling good matters physiologically. Relaxed body conditions create favorable chemistry across the crucial seven days surrounding ovulation. A loving, unhurried contact does more for conception than any anxious, obsessively logged temperature reading ever truly could.</p>
<p class="mb-4 font-body-md text-on-surface">Getting closer emotionally shifts biology. Partners who stay loving and present, skin contact frequent, report calmer cycles. The woman's body rewards safety, not surveillance; that is the truth most fertility apps quietly refuse to mention.</p>
<p class="mb-4 font-body-md text-on-surface">Trust your own bodies over borrowed rules. Feeling good, low worry, steady contact every few days, this natural process carries couples through seven days of possibility while the rigid tracking process crowd stays anxious enough.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">How The Tool Estimates Your Dates</h2>
<p class="mb-4 font-body-md text-on-surface">Most people assume you can pinpoint exactly when you became pregnant. Biologically, that's difficult. A pregnancy conception date varies across women because multiple ways exist to measure it, and each gives slightly different conception dates.</p>
<p class="mb-4 font-body-md text-on-surface">A conception calculator works by entering dates — usually the mother's last menstrual period — then applying the average menstrual cycle length. From these factors it estimates when real conception most likely happened for you.</p>
<p class="mb-4 font-body-md text-on-surface">This is why a pregnancy calculator stays the number one, most popular starting point for anyone unsure how many weeks pregnant they are, or trying to track pregnancy week by week.</p>
<p class="mb-4 font-body-md text-on-surface">The age of the baby — its gestational age on date selected — differs from the moment conception took place by roughly two weeks, since clinicians count from your last period, not fertilization.</p>
<p class="mb-4 font-body-md text-on-surface">Whether it's a planned baby or an unplanned pregnancy, the woman's body works identically. Sperm linger 3-5 days, while implantation follows conception by roughly 10 days, shifting your fertile map.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">When Did I Conceive?</h3>
<p class="mb-6 font-body-md text-on-surface">At your first prenatal visit, the clinician often estimates when you ovulated. If your period started April 24, ovulation near May 7 — about 13 days later — marks the likeliest window.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Estimating Conception From An Ultrasound</h2>
<p class="mb-4 font-body-md text-on-surface">Beyond dating methods built purely on cycles, a last ultrasound reads the embryo directly, checking its progress and the health of both mother and child through precise, measurable early growth.</p>
<p class="mb-4 font-body-md text-on-surface">Timing is everything here. On a 30-day cycle, a scan reading 38 days points to conception about 16 days after your period — noticeably earlier than clumsy tracking ovulation guesswork suggests.</p>
<p class="mb-4 font-body-md text-on-surface">Software then back-calculates the fuller picture: a January 15 scan showing 54 days of growth sets conception near December 8, sharpening ovulation timing into a single, dependable, dated estimate you can trust.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">IVF, Delivery, And Why Ranges Beat Single Dates</h2>
<p class="mb-4 font-body-md text-on-surface">For IVF embryo transfer, the calculator skips guesswork entirely — the transfer date fixes conception precisely. Natural pregnancy timing windows stay wider, which is why the tool shows ranges, not one rigid day.</p>
<p class="mb-4 font-body-md text-on-surface">That range matters because few babies actually deliver on the exact calculated date. Knowing your true conception window simply helps you plan care, testing, and expectations across the pregnancy realistically.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How Accurate Is the Due Date?</h3>
<p class="mb-6 font-body-md text-on-surface">Treat your due date as a probability, not a promise. Fewer than one in twenty babies arrive on it. Still, it anchors pregnancy planning and early preparations with useful, workable structure.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">Can My Due Date Change?</h3>
<p class="mb-6 font-body-md text-on-surface">Due dates change; that's normal. A shift means accuracy, not error. Fetal measurements convert your cycle-based general estimate into due date adjustment. New information, baby's growth, natural variability nudge pregnancy toward its actual due date.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">What If I Already Know My Due Date?</h3>
<p class="mb-6 font-body-md text-on-surface">Knowing the date is the beginning, not the answer. Entering it into the Pregnancy Due Date Calculator stays helpful, generating a detailed pregnancy timeline with key milestones, symptoms, prenatal tests, reminders, and scheduled prenatal visits.</p>
<h2 class="font-bold text-primary mt-10 mb-4 text-display-sm">Gestational Age at Delivery by Plurality</h2>
<p class="mb-4 font-body-md text-on-surface">Forty weeks fits singleton pregnancies alone. Multiple gestations deliver earlier: twins 35 weeks, triplets 32 weeks, quadruplets 30 weeks, quintuplets 27 weeks. Plurality sets the average gestational age assigned, while the singleton reaches 39 weeks.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How is the estimated due date calculated?</h3>
<p class="mb-6 font-body-md text-on-surface">Most tools apply Naegele's rule: add 280 days to your last menstrual period, assuming a 28-day cycle and ovulation around day fourteen. That fixed formula yields forty weeks of gestation.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">What if I don't know my last period / LMP date?</h3>
<p class="mb-6 font-body-md text-on-surface">Irregular or unknown? A dating ultrasound or clinical scan measures baby's length, yielding ultrasound gestational age. That becomes an accurate due date, established due date, or known conception date to estimate EDD and pregnancy timeline.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How accurate is an estimated due date?</h3>
<p class="mb-6 font-body-md text-on-surface">Think helpful target, never fixed deadline. Only about four percent are born on the date. Most healthy pregnancies are delivered full-term, anywhere between 37 and 42 weeks, and that spread is completely normal.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">Can my due date change during pregnancy?</h3>
<p class="mb-6 font-body-md text-on-surface">Your calendar is a roadmap, not a contract. As your baby grows, ultrasound measurements and fresh clinical data let your provider track development and, when warranted, the estimate is adjusted.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How many weeks pregnant am I today?</h3>
<p class="mb-6 font-body-md text-on-surface">The clinical standard, measured from last period, makes you two weeks pregnant before conception occurs. Gestational weeks precede the day of conception. Your doctor uses this timeline to set your prenatal appointment schedule for pregnancy.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">Can I find out the exact day I got pregnant?</h3>
<p class="mb-6 font-body-md text-on-surface">Rarely with certainty. Sperm can live up to five days, so the fertile window, not one exact day, is when you likely got pregnant. Conception timing varies, which is why it's estimated, never precisely calculated.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">What is conception date?</h3>
<p class="mb-6 font-body-md text-on-surface">It's the day sperm fertilized the egg — biologically when you actually became pregnant. Unlike the LMP-based estimate, it reflects the real fertilization event, though it usually can't be pinpointed to a single calendar day.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How long after conception until pregnancy officially begins?</h3>
<p class="mb-6 font-body-md text-on-surface">Fertilization isn't the official start. The fertilized egg travels for roughly 6-12 days before it can implant in the uterine wall. Only after that implantation, clinically speaking, does pregnancy officially begin.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How does ultrasound dating calculate a due date?</h3>
<p class="mb-6 font-body-md text-on-surface">It measures the embryo or fetus — crown-rump length early on — and converts that size into gestational age expressed in weeks and days, then projects forty weeks forward to estimate delivery.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">When should ultrasound dating be preferred over LMP?</h3>
<p class="mb-6 font-body-md text-on-surface">When a discrepancy between LMP dating and ultrasound exceeds recommended thresholds — often five to seven days in the first trimester — the ultrasound estimate should replace the period-based date for accuracy.</p>
<h3 class="font-bold text-primary mt-10 mb-4 text-title-lg">How is EDD calculated for IVF/ART?</h3>
<p class="mb-6 font-body-md text-on-surface">With IVF, dating is unusually precise because the embryo's age is known. Providers add 266 days to egg retrieval for fresh transfers, or adjust by the embryo's age at transfer for frozen cycles.</p>
